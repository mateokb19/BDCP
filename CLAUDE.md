...
ServiceCategory:    exterior | interior | ceramico | correccion_pintura | latoneria | pintura | ppf | polarizado | otro
...
-- Seed: UPDATE services SET price_moto = 40000, price_automovil = 51000 WHERE name = 'Premium Wash' AND category = 'exterior';
...
1. **IngresarServicio** — 3-step wizard:
   ...
   - **Service selection (step 2)**: services column shows collapsible accordion sections — Detallado (Exterior / Interior / Corrección / Cerámico), Latonería, Pintura, PPF, Polarizado, Otros servicios. "Detallado" is open by default; all others start closed. Services from multiple areas can be combined in one order. Each accordion header shows a count badge when services are selected from that area.
     ...
     - **Moto pricing**: uses `price_moto` column for display price (what client pays). `standard_price` in `service_order_items` is set to `price_automovil` for moto orders so detallado commission is based on the automóvil catalog price ($51,000 / $85,000). `price_moto` column added to `services` table via migration.
     - **PPF / Polarizado**: seeded with price $0; user enters a custom price per service in step 2 (editable input). Quantity selector is **informational only** — it does NOT multiply the price. The user enters the total price directly.
     ...
     - **Otros servicios**: `category="otro"` — fixed services: PDR ($200k), Arreglo de Rin ($250k), Lavado Motor ($60k), Lavado Chasis ($60k), plus 5 custom "Otro servicio" slots for ad-hoc items. Not mapped to any operator type — never count toward any operator's liquidation.
   - **IVA display (IngresarServicio)**: detallado category services (exterior, interior, ceramico, correccion_pintura) already include IVA — no extra display. All other categories (latoneria, pintura, ppf, polarizado, otro) do NOT include IVA — the UI shows "+IVA $X" per service in step 2 and a total IVA line in step 3. IVA is **informational only** — never added to the order total. Helpers: `DETALLADO_CATS = new Set([...])`, `serviceNeedsIva(service)`, `ivaOf(amount)`.
...
3. **EstadoPatio**: fetches `GET /api/v1/patio` on mount. Kanban cards advance via `POST /patio/{id}/advance`. Services editable via `PATCH /patio/{id}`. Operator assigned via modal on first advance.
   ...
   - **Editar orden** modal:
     ...
     - **All 9 service categories** available in the add-services accordion: Exterior, Interior, Corrección, Cerámico, Latonería, Pintura, PPF, Polarizado, Otros.
   ...
   - Delivery date picker in the inline editor: dark-themed via `style={{ colorScheme: 'dark' }}` + `[&::-webkit-calendar-picker-indicator]:invert`. Hour selector shows 8:00–17:00. Layout uses `flex flex-wrap gap-2` so the hour selector wraps below the date input on narrow cards.
   - **Advancing listo → entregado**: intercepts with a payment modal:
     ...
     - **IVA toggle**: discrete "Aplicar IVA (19%)" checkbox in the payment modal amount summary. Default = OFF. When ON: adds 19% IVA **only on non-detallado items** (`nonDetailladoTotal` = sum of `unit_price * quantity` for items whose `service_category` is not in `DETALLADO_CATS`). The `effectiveRestante = applyIva ? restante + ivaAmt : restante` is used for payment splitting and saved to the order. Resetting pay methods when IVA toggled to avoid inconsistent splits. Critical: IVA is computed from item categories, NOT from `restante * 1.19` (which would incorrectly include detallado services).
...
- **`_seed_if_empty()`** reseeds services if `count != 62` (restart-to-update pattern). Operators are seeded with per-name INSERT-if-missing (not `count == 0`), so new operators can be added to the seed list without wiping existing data. 62 services span 9 categories: exterior (11), interior (9), correccion_pintura (4), ceramico (5), ppf (2), polarizado (1), pintura (11), latoneria (10), otro (9 — 4 fixed + 5 custom slots).
...
- **CalendarioCitas UX**: past days in month grid are dimmed (`text-gray-700`). New appointment date picker has `min=today`; edit mode has no minimum (allows changing to any date). Time picker is a `<select>` with full hours 6:00–18:00. Appointments within a day are sorted by time ascending. Field order in form: Marca → Modelo → Placa. Brand and model fields are `<input type="text">` with `<datalist>` (not `<select>`) to allow free typing on mobile. BRANDS and MOTO_BRANDS lists sorted alphabetically. On Enter/blur, typed value normalizes to exact catalog spelling (case-insensitive match). New/edit cita modal uses `closeOnOverlay={false}` so switching apps on mobile doesn't accidentally dismiss the form.
...
- **`Modal` `closeOnOverlay` prop**: `closeOnOverlay?: boolean` (default `true`). Pass `false` to prevent the overlay click from closing the modal — used in CalendarioCitas new/edit cita modal so switching apps on mobile doesn't dismiss the form.
...
- **`NO_OPERATOR_TYPES`** (EstadoPatio): `const NO_OPERATOR_TYPES = new Set(['ppf', 'polarizado', 'otro'])`. Orders whose service categories are entirely in this set advance from `esperando → en_proceso` without any operator picker. The operator validation in the edit modal and the "Guardar" button also skip these types. Rationale: PPF/Polarizado/Otros are paid to third parties manually or are misc services — no internal operator needed.
