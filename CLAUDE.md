...
3. **EstadoPatio**: fetches `GET /api/v1/patio` on mount. Kanban cards advance via `POST /patio/{id}/advance`. Services editable via `PATCH /patio/{id}`. Operator assigned via modal on first advance.
   ...
   - **Bodega minimap**: sticky panel (200×240px floor plan, visible `xl+` only) to the right of the kanban columns. Shows a proportional floor plan of the workshop with 5 zones drawn as CSS lines: (1) Pintura/Latonería — 2 slots with solid vertical center divider; (2) Lavado fila 1 — 3 cols; (3) Lavado fila 2 — 3 cols (zones 1–3 separated by dashed horizontal lines); (4) Detallado — 3 slots left col + 1 right; (5) Cuarto inferior — 2 slots left col + 1 right (separated from main body by a solid horizontal line). 15 vehicle slots total as absolute-positioned 10×10px dots; slots fill in order as vehicles arrive. Occupied slots animate in with a spring scale, empty slots show as a faint border. Counter + progress bar color: green (<10) → yellow (10–14) → red (≥15). Overflow badge shows `+N` when >15 active vehicles.
